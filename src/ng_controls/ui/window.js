...
(function(window) {                  
   ...
   */
  window.ngCreateWindow = function(def,parent)
  {
    if(!def) return null;
    var cnt=0,wname='';
  ...
  {  
    if(!this.IsDragEvent(pi.EventID)) 
    {
      this.MouseType = -1; 
      return;
    }
  ...
  {
    if(this.AutoSize) {
      if(this.ngw_inautosize<(ngIExplorer6 ? 2 : 1))
      {
        this.ngw_inautosize++;
        try {
          this.CalcAutoSize();
   ...
   */
  window.ngCreateHint = function(def,parent)
  {
    ...
    return window.ngCreateWindow(def,parent);
  }
   ...
   *    Created hint object (<ngHint>). 
   */
  window.ngPopupHint = function(def,x,y,anchor,parent)
  {
    var h=window.ngCreateHint(def,parent);
   ...
   *    Created hint object (<ngHint>). 
   */
  window.ngPopupCtrlHint = function(def,ctrl,anchor,parent)
  {
  ...
  function ngh_SetVisible(v)
  {
    ...
    else
    {
      ...
      if((ht>0)&&(!this.InDesignMode)) {
        var self=this;
        this.HintAutoHideTimer = setTimeout(function() {
          if(self.SetVisible) self.SetVisible(false);
        },ht);
      }
    }
  }
   ...
   *    Created hint object (<ngHint>). 
   */
  window.ngCreateTextHint = function(def,text,parent)
  {
    if(typeof def==='string') def={Type: def};
    ng_MergeDef(def, {
      Data: {
    ...
    return window.ngCreateHint(def,parent);
  }
   ...
   *    Created hint object (<ngHint>). 
   */
  window.ngPopupTextHint = function(def,x,y,text,anchor,parent)
  {
    var h=window.ngCreateTextHint(def,text,parent);
    if(h) h.Popup(x,y,anchor);
    return h;
  }
   ...
   *    Created hint object (<ngHint>). 
   */
  window.ngPopupCtrlTextHint = function(def,ctrl,text,anchor,parent)
  {
    ...
    function Create_ngTextHint(def, ref, parent)
    {
      ng_MergeDef(def, {
        ...
        */
        ...
        Methods: {
          DoUpdate: nghtxt_DoHintUpdate,
          DoPtrClick: nghtxt_DoPtrClick,
          DoMeasureText: nghtxt_DoMeasureText
        },
        ControlsPanel: {
          Data: {
            Gestures: {
      ...
      return ngCreateControlAsType(def, 'ngHint', ref, parent);
    }
